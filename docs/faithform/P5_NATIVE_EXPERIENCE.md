...
- `apps/faithform-ios/Sources/FaithFormKit/Strings.swift` holds every user-facing
...
Prompt 4's fail-closed router, unchanged. `faithform://church/{slug}/announcements`
...
| Channels | none (iOS has no equivalent) | `faithform_announcements`, `faithform_events`, created at launch |
...
non-`faithform://` link is refused. The payload is a hint, never a way in.
