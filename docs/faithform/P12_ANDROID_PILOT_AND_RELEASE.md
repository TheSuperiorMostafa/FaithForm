...
| --- | --- | --- | --- |
| Application id | `…faithform.dev.debug` | `…faithform.dev.staging` | `…faithform.dev` |
...
```bash
./gradlew :app:assembleStaging -Pfaithform.stagingOrigin=https://staging.example.test
./gradlew :app:assembleRelease  -Pfaithform.releaseOrigin=https://faithform.io
```
...
An unconfigured build **builds no object graph at all**. `FaithFormApplication`
...
cd apps/faithform-android
./gradlew :app:assembleStaging -Pfaithform.stagingOrigin=https://…   # pilot APK
./gradlew :app:assembleRelease -Pfaithform.releaseOrigin=https://…   # unsigned
```
