...
*What already exists, what is authoritative, and what FaithForm is allowed to add.
...
**FaithForm already has a complete Stripe Connect giving system.** FaithForm adds a
...
That matters for FaithForm: there is nothing to hand a phone. A mobile "receipt"
...
FaithForm must be bound to the authenticated account.

---

## 3. What is authoritative, and stays that way

| Authority | Owner | FaithForm's relationship to it |
| --- | --- | --- |
...
| Funds and campaigns | `giving_funds` | FaithForm adds publication columns, nothing else |
| Donor identity | `giving_donors`, per church | FaithForm adds an explicit account link |
...
2. **No link from a FaithForm account to a `giving_donor`.** `giving_donors` is
...
what the prompt explicitly forbids. So FaithForm gives one-time, tells the visitor
...
existing dashboard policy explicitly supports it*. It does not, so FaithForm does
not offer it.
