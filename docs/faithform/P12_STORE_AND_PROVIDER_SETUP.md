...
| Bundle identifier | replace `io.faithform.placeholder.faithform`. Set `FAITHFORM_BUNDLE_ID_PREFIX` |
...
1. Create a Merchant ID: `merchant.<your prefix>.faithform`.
2. Register it with Stripe so Stripe can decrypt the payment token.
3. Add `com.apple.developer.in-app-payments` to `FaithForm.entitlements`.
...
The honest headline is that FaithForm collects a small amount and links all of it,
...
| Step | Detail |
| --- | --- |
| Package name | replace `io.faithform.app.dev` |
