...
For `0054_faithform_publication_and_push.sql`, the notification worker, and the
APNs/FCM providers.
...
```bash
FAITHFORM_MIGRATION_DATABASE_URL=... \
FAITHFORM_MIGRATION_CONFIRM=i-understand-this-is-not-production \
pnpm db:faithform-push
```
...
Notification channels (`faithform_announcements`, `faithform_events`) are created
...
`faithform://church/{slug}/announcements` works today via the custom scheme.
