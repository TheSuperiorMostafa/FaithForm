...
## AD-001 — FaithForm owns administration; FaithForm is a constrained native client
...
- FaithForm owns only the signed-in visitor’s profile/preferences/consent/cache and allowed requests: discover/follow/join, read published content, submit attendance attempts, and initiate payment operations.
- Both use one shared backend. FaithForm never owns a parallel church, People, attendance, content, or donation database.
...
- A FaithForm action must appear in existing FaithForm views through the same authoritative record.
...
`lib/faithform/relationship-state.ts`. The questions this decision listed under
...
- FaithForm never reads dashboard draft/editor tables as an unbounded generic client.
...
- FaithForm lists and renders published versions read-only. It does not call editor/generation endpoints or reconstruct from mutable live theme/scripture data.
...
- Stripe Connect remains the payment provider. FaithForm/FaithForm servers create short-lived payment/setup operations with provider idempotency; native apps use the supported Stripe SDK or hosted surface.
...
Add a repository-owned platform-neutral area such as `design/faithform/` containing versioned semantic tokens, component/state specifications, motion durations/curves, icon/image references, accessibility semantics, and golden fixtures. Generate or verify native constants from it. This path is a recommendation, not an implementation in this audit.
...
4. FaithForm reads only published/versioned content.
