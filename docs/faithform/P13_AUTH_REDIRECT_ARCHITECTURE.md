...
| | FaithForm dashboard (web) | FaithForm (iOS + Android) |
|---|---|---|
...
[`contracts/faithform/v1/auth-callback.json`](../../contracts/faithform/v1/auth-callback.json).
...
dashboard. So a visitor confirming their email from FaithForm was delivered to
...
protocol-relative `//host`, a backslash trick, or `faithform://…` all collapse to
`/dashboard`.
...
sent everyone without a `church_users` row back to `/login`. A FaithForm visitor
...
- iOS `FaithFormLog` accepts a `StaticString` event name, a request id, and a
...
- `apps/faithform-ios/App/Resources/Info.plist` registers `CFBundleURLSchemes`
  = `faithform`.
...
built. Debug reads `FAITHFORM_DEV_SUPABASE_URL` / `FAITHFORM_DEV_SUPABASE_ANON_KEY`
...
```
FAITHFORM_API_ORIGIN, FAITHFORM_SUPABASE_URL, FAITHFORM_SUPABASE_ANON_KEY
```
...
- `apps/faithform-android/app/src/main/AndroidManifest.xml` declares the
  `android:scheme="faithform"` intent filter on `MainActivity`, which is
...
```
-Pfaithform.stagingOrigin=…  -Pfaithform.releaseOrigin=…
-Pfaithform.supabaseUrl=…    -Pfaithform.supabaseAnonKey=…
```
...
2. **Create an account** in FaithForm with a real address. Confirm the app shows
   "Check your email", not an error.
3. **Inspect the email before tapping.** Long-press the button and read the URL.
   It must contain `redirect_to=faithform%3A%2F%2Fauth%2Fcallback`. *If it points
   at the dashboard origin, the allow-list row in §4.1 is missing — stop and fix
   that; nothing downstream can compensate.*
4. **Tap the link.** Expected: the browser opens for a moment, then **FaithForm
...
| Check | Result |
|---|---|
| `swift test` (FaithFormKit) | 374 tests, 48 suites — pass |
