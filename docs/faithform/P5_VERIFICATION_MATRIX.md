...
| 2 | Invitation onboarding uses no staff membership | no FaithForm module touches `church_users` — asserted | ✅ |
...
| 50 | iOS push entitlement declared | `Config/FaithForm.entitlements`, both xcconfigs | ✅ |
...
| ├ `faithform-publication-migration` | 20 |
| ├ `faithform-push` (adapters, dedupe) | 14 |
| ├ `faithform-discovery-authorization` | 23 |
| ├ `faithform-provider-auth` (ES256, RS256, refresh, redaction) | 19 |
| └ `faithform-worker-invocation` (cron, lease, retry) | 17 |
| iOS (`swift test`) | 99 |
