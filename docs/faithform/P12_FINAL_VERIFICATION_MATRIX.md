...
| iOS library build (device) | `pnpm ios:build:device` | **clean** — now delegates to the app build, which compiles FaithFormKit for iOS as a dependency (§4.6) |
...
| --- | --- |
| `pnpm ios:app:generate` | generates `FaithForm.xcodeproj` from `project.yml` |
...
compiled the package with `xcodebuild -scheme FaithForm`, and with signing enabled
...
app build instead; the app depends on FaithFormKit, so the check it exists for is
still performed.
