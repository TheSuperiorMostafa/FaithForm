# FaithForm / FaithForm implementation sequence
...
Establish the only identity/tenant model FaithForm will use while preserving existing `churches`, `church_users`, and `members` authorities.
...
Deliver the first read-heavy FaithForm vertical: discover/follow/join churches and receive published announcements/events with artwork and user-controlled notifications.
