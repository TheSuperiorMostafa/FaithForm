...
| **No payment sheet at launch** | `canPresentSheet` is false in every launch state; a source sweep asserts `MainActivity`, `FaithFormApplication` and `AppViewModel` never present, build or configure a sheet |
...
`publish_recording_to_faithform` had picked up an audit insert the caller already
...
| **An installable iOS app** | `apps/faithform-ios` is a SwiftPM library: no `@main`, no `Info.plist`, no Xcode project, no signing identity | runbook §8 |
...
FaithForm attempt and leaves the donation write, the receipt path and every
