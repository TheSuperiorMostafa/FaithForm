...
`apps/faithform-ios` was a SwiftPM library. It now also has an application.

```
apps/faithform-ios/
  project.yml                     ← the app target, as a readable spec
  App/
    FaithFormApp.swift             @main
    ...
    Resources/
      ...
      FaithForm.entitlements       push only
...
pnpm ios:app:generate     # then open apps/faithform-ios/FaithForm.xcodeproj
```
...
it produces is a `FaithForm.app` that cannot be installed, which is the correct
...
`xcodebuild -scheme FaithForm`; a `FaithForm.xcodeproj` now sits beside the package
...
depends on FaithFormKit, so FaithFormKit is compiled for iOS either way, under the
...
```bash
xcodebuild ... FAITHFORM_API_ORIGIN=https://staging.example.test
```
...
`io.faithform.placeholder.faithform`, and deliberately obvious. It is meant to be
overridden:

```
FAITHFORM_BUNDLE_ID_PREFIX = com.yourorg
```
...
* **Associated domains.** App Links need a verified
  ...
  The `faithform://` scheme works today; https links do not, and declaring the
...
```bash
cd apps/faithform-ios/App/Configuration
cp Local.xcconfig.example Local.xcconfig
# edit it: DEVELOPMENT_TEAM and FAITHFORM_BUNDLE_ID_PREFIX
...
1. Open `apps/faithform-ios/FaithForm.xcodeproj`.
2. **FaithForm** target → **Signing & Capabilities**. It should already show your
