...
`visitor_accounts.avatar_url` is the source of truth for both apps and web member/directory queries. Mobile refreshes bootstrap after saving. Dashboard navigation avatars fetch the signed-in user's record through a cookie-authenticated endpoint on load, on window focus/visibility, and every 30 seconds while visible. The People list resolves each person's photo the same way: `listAppPhotos` reads the church's active `visitor_people_links` and the accounts they name with the service role — row security keeps an account's row to itself, so a link alone cannot reach it — and the People record's own `photo_url` is the fallback. A photo belongs to the person, not to the church's app plan, so it is loaded whether or not the `member_app` flag shows who is on the app. Groups and directory rows already resolved it this way through `labelMemberships`. It is eventual sync, not a realtime subscription. Failed image loads fall back to initials.
