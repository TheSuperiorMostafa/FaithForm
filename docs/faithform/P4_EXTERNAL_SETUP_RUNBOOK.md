...
Everything FaithForm needs that this repository cannot establish for itself.
...
|---|---|---|---|
| iOS bundle id | `io.faithform.app.dev` | Real id, registered in the Apple Developer account | Apple account holder |
| Android application id | `io.faithform.app.dev` (`.debug` suffix on debug) | Real id — **immutable after first Play upload** | Play account holder |
| iOS Team ID | unset | Required for signing and Universal Links | Apple account holder |

Configured in `apps/faithform-ios/Config/Production.xcconfig` and
`apps/faithform-android/app/build.gradle.kts`.
...
Working today: the `faithform://` custom scheme, on both platforms, parsed and
...
Until a provider is chosen, `FaithFormLog` and Android logging emit only event
names, request ids, and typed error codes.

## 8. Privacy declarations

`apps/faithform-ios/Config/PrivacyInfo.xcprivacy` exists and is **accurate for
