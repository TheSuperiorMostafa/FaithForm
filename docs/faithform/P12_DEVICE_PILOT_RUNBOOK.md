...
1. A first-run screen on each platform: **Create Account** (email + password),
   **Sign In**, and **Forgot password**, against Supabase's GoTrue endpoints
   directly (`SupabaseAuthClient` in FaithFormKit / `SupabaseAuthClient.kt` in
...
2. Configuration on both platforms, empty by default and failing closed:
   `FAITHFORM_SUPABASE_URL` / `FAITHFORM_SUPABASE_ANON_KEY` on iOS (fed from the
   gitignored `Local.xcconfig` in Debug), `faithform.supabaseUrl` /
   `faithform.supabaseAnonKey` Gradle properties or `local.properties` on
   Android.
...
including one held across sign-in from a `faithform://invite/<token>` link);
