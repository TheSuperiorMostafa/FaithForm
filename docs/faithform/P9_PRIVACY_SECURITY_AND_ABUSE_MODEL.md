...
| A website `?cap=` replayed at a FaithForm route | a different derived sub-key — it does not verify |
| A FaithForm capability replayed at the website route | the same, in reverse |
...
FaithForm reuses it unchanged. No new table, no per-person watch history, and no
