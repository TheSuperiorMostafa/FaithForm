...
| `church_users` | FaithForm | **Dashboard access.** No FaithForm module writes this table — asserted by test |
| `visitor_accounts.id` | FaithForm | A profile. Never dashboard access |
| `members.id` | The church | The operational person. Never created, merged, or deleted by any FaithForm path |
| `giving_donors.id` | The church | Untouched by Prompt 3 |

`grantsDashboardAccess()` (`lib/faithform/relationship-state.ts:171`) is total and
...
and testable (`lib/faithform/relationship-state.ts`); every write funnels through
one `applyTransition` (`lib/faithform/relationships.ts:104`).
...
(`approveClaim`, `lib/faithform/people-claims.ts:565`), it requires an explicit
...
`lib/faithform/` (errors, relationship-state, schemas, invitation-token, account,
discovery, relationships, staff-relationships, people-claims, campuses,
account-lifecycle, invitations); `lib/queries/faithform-settings.ts`;
`app/dashboard/settings/faithform-actions.ts`;
`app/dashboard/people/claim-actions.ts`;
`components/settings/faithform-visibility-card.tsx`;
