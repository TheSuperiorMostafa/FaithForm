...
| Requirement | Observed |
| --- | --- |
| Publication controls and audit record | `publishToFaithForm` writes an audit row with the actor; four actions asserted |
...
`#if os(iOS)` block in `FaithFormKit` was therefore invisible to them — the camera
...
Migration 0062's `publish_recording_to_faithform` picked up an audit insert while
...
| `create function … security definer` with no `search_path` | `0061_faithform_media_eligibility.sql` | migration baseline scanner (§4.8) |
| `create function public.consume_api_rate_limit` | `0061_faithform_media_eligibility.sql` | baseline-shadowing check (§4.8) |
...
| **An installable iOS app** | `apps/faithform-ios` is a SwiftPM *library*. There is no app target, no `@main`, no `Info.plist`, no Xcode project and no signing identity, so nothing can be installed on a device | `P4_EXTERNAL_SETUP_RUNBOOK.md` |
