...
| Offline | "FaithForm could not reach the server", with a retry |
...
FaithForm gives **one-time**. The existing dashboard and Stripe model support
...
**Stripe's own.** `PaymentSheet` on both platforms. FaithForm has no card field,
...
**There is no iOS app target.** `apps/faithform-ios` is a SwiftPM library — no
