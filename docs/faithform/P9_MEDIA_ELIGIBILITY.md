...
*Why "published to FaithForm" now means "playable by FaithForm", how eligibility
...
`frma` says was underneath. FaithForm ships no key acquisition and no CDM, so a
...
every recording already published to FaithForm **invisible** — in the list, in
...
| **In FaithForm** | published *and* playable | present, to change |

A recording that was published and has since been proved unplayable stops
showing **In FaithForm**, because it is not: the projections stopped serving it
