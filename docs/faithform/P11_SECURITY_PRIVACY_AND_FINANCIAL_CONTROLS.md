...
`giving_donors` is keyed `(church_id, email)`. Matching a FaithForm account to a
...
**FaithForm has no card field.** Card details are entered inside Stripe's own
...
`FinancialConnections`, `USBankAccount` and `STPBankAccount` are absent. FaithForm
...
every other FaithForm cache. A church switch, a sign-out, an authorization-version
