...
* No app installed on a phone. **There is no iOS app target at all** —
  `apps/faithform-ios` is a library.
...
A fund cannot be published to FaithForm unless its church has
...
and `charge.dispute.*` now also project onto a FaithForm attempt.
...
**There is none.** `apps/faithform-ios` is a SwiftPM library: no `@main`, no App
...
Stripe's SDKs handle their own redirect returns, so FaithForm adds no custom
...
donation. FaithForm shows the same gift in-app and **sends no email of its own**.
