...
| ---: | --- | --- | --- |
| 1 | Visitor accounts are separate from `church_users` | `visitor-authorization.test.ts` — "no FaithForm module ever writes church_users" | ✅ |
| 2 | Following/joining cannot grant dashboard access | `faithform-relationship-state.test.ts` — asserted for every state | ✅ |
| 3 | Visitor profile creation is idempotent | `lib/faithform/account.ts:70-110`; unique `user_id` + conflict re-read | ✅ |
...
| 8 | All three join policies work | `faithform-relationship-state.test.ts` — open/approval/invite_only | ✅ |
...
| 15 | Tokens stored hashed, never raw | `visitor-authorization.test.ts`; `faithform-identity.test.ts` | ✅ |
...
| 17 | Duplicate email/phone across unrelated People | `faithform-identity.test.ts` — asserts collision is expected | ✅ |
...
| 27 | Campus coordinates validated | schema refine + DB check constraints; `faithform-identity.test.ts` | ✅ |
