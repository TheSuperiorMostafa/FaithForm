...
**`Config/FaithForm.Info.plist`:** `NSCameraUsageDescription` only.

```
FaithForm uses your camera to read the check-in code your church shows on
...
`required="false"` matters: `true` would remove FaithForm from the Play listing for
