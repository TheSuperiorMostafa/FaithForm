...
| --- | --- | --- |
| **Not in FaithForm** | the default for everything | yes |
...
| **In FaithForm** | visible to the people the church chose | yes (to change) |
| **Removed from FaithForm** | a staff member took it down | yes (to restore) |
...
**In FaithForm**, because it is not in FaithForm — the projections stopped serving
...
| Option | Sees it |
| --- | --- |
| Anyone using FaithForm | everyone, including someone who merely found the church |
...
> *"Publishing to FaithForm isn't set up on this FaithForm installation yet. Ask
...
```bash
createdb faithform_rehearsal
psql faithform_rehearsal -f tests/database/fixtures/bootstrap.sql
for n in 0055 0056 0057 0058 0059 0060 0061 0062; do
  psql faithform_rehearsal -f supabase/migrations/${n}_*.sql
done

FAITHFORM_TEST_DATABASE_URL=postgres://…/faithform_rehearsal pnpm test:concurrency
```
...
defaults to false, so every recording already in FaithForm disappears from the
...
FaithForm's plays are recorded through the **existing** `media_views` mechanism
...
| A recording that was in FaithForm vanished from the app | it was re-probed and is no longer playable. The church's intent is intact — fix the file and it returns by itself |
