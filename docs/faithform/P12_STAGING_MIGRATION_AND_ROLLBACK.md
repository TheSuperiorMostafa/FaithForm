...
Nine FaithForm migrations sit on top of the pre-existing schema:
...
```bash
FAITHFORM_PG_HOST=localhost pnpm pilot:rehearse
```

Creates a fresh disposable database, applies **every** FaithForm migration on
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| App shows "not set up" | the build has no origin | supply `FAITHFORM_API_ORIGIN` / `-Pfaithform.…Origin` |
