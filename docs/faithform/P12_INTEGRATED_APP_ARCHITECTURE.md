...
| Concern | Where | State |
| --- | --- | --- |
| Typed API client, envelope, error codes | `FaithFormKit/Networking`, `:core:network` | complete |
| Cache partition `environment · account · church · authorizationVersion` | `CachePartition.swift`, `:core:storage` | complete |
| Destination enum, deep-link parser, `RouteRegistry` | `FaithFormKit/Navigation`, `:core:navigation` | complete, and already gated on four independent conditions |
...
| Android app host | `:app` — `MainActivity`, `FaithFormApplication`, manifest, receivers | **real**, and reaching only the account shell |
...
**2. `apps/faithform-ios` was a library, not an app.**
...
**FaithForm ships with sermons absent**, the destination stays unregistered, and
...
| | iOS | Android |
| --- | --- | --- |
| Entry | `@main struct FaithFormApp` | `FaithFormApplication` + `MainActivity` |
...
| Root | `RootView` | `FaithFormApp` composable |
