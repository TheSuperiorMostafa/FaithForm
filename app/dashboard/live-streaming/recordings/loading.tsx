...
/**
 * Mirrors the Recordings tab: the four filter chips (real labels, counts
...
export default function RecordingsLoading() {
  return (
    <SkeletonContainer className="flex w-full flex-col gap-6" label="recordings">
      <div className="flex flex-wrap gap-2">
        {RECORDING_FILTERS.map((filter, index) => (
          ...
          >
            {filter.label}
            {filter.key !== "series" ? <Skeleton className="h-6 w-7 rounded-full" /> : null}
          </span>
        ))}
      </div>
      ...
      <ul className="divide-y divide-border rounded-3xl border border-border bg-card p-2 shadow-sm">
        {Array.from({ length: 5 }).map((_, index) => (
          <li key={index} className="flex flex-col gap-3 p-2 sm:flex-row sm:items-center sm:gap-4">
