...
export type FaithFormActionResult<T = undefined> =
  | { ok: true; data: T }
...
> {
  ...
  if (!auth.isAdmin) {
    return { ok: false, error: "Only a church admin can publish to FaithForm." };
  }
...
const VISIBILITY_LABELS: Record<Exclude<MobileVisibility, "none">, string> = {
  public: "Anyone using FaithForm",
  followers: "People following this church",
  members: "People who have joined this church",
};

export async function getFaithFormPublishingState(): Promise<
  FaithFormActionResult<{
...
): Promise<FaithFormActionResult<PosterChoice[]>> {
  const auth = await requireAdmin();
  if (!auth.ok) return auth;
...
export async function publishMediaToFaithForm(input: {
  kind: "live" | "recording";
  id: string;
  visibility: Exclude<MobileVisibility, "none">;
  posterUrl?: string | null;
  summary?: string | null;
}): Promise<FaithFormActionResult<{ state: string }>> {
  const auth = await requireAdmin();
  if (!auth.ok) return auth;

  const result = await publishToFaithForm({
    ...
    return {
      ...
      error:
        ...
        result.reason === "not_playable"
          ? (result.explanation ??
            "FaithForm can't play this recording. It needs converting first.")
          : // The file in storage is not the file that was checked. Said plainly,
            ...
            ? "This recording's file changed since it was checked. FaithForm is checking it again — try publishing in a minute."
...
export async function unpublishMediaFromFaithForm(input: {
  kind: "live" | "recording";
  id: string;
  revoke?: boolean;
}): Promise<FaithFormActionResult<{ state: string }>> {
  ...
  const result = await unpublishFromFaithForm({
    churchId: auth.churchId,
    kind: input.kind,
    id: input.id,
    revoke: input.revoke,
...
export async function getFaithFormPublicationHistory(input: {
  kind: "live" | "recording";
  id: string;
}): Promise<FaithFormActionResult<AuditEntry[]>> {
  const auth = await requireAdmin();
  if (!auth.ok) return auth;
 ...
 */
export async function previewFaithFormVisibility(): Promise<
  FaithFormActionResult<{
    slug: string | null;
