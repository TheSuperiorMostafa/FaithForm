...
let package = Package(
    ...
    products: [
        .library(name: "FaithFormKit", targets: ["FaithFormKit"])
    ],
    dependencies: [
        // Stripe's own payment sheet. The only place a card number is ever
        // entered is inside Stripe's UI — FaithForm has no card field.
    ...
    targets: [
        ...
        .testTarget(
            name: "FaithFormKitTests",
            dependencies: ["FaithFormKit"],
            path: "Tests/FaithFormKitTests",
            swiftSettings: [.swiftLanguageMode(.v6)]
        )
    ]
)
