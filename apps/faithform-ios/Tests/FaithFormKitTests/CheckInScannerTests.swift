...
///
/// The point of the abstraction: "FaithForm never asks for the camera until you
...
struct PayloadReadingTests {
    ...
    @Test("a FaithForm token is recognised")
...
struct CheckInRoutingTests {
    ...
    func parsesCheckInLink() {
        let url = URL(string: "faithform://church/grace-chapel/check-in")!
    ...
    func linkStartsNothing() async {
        ...
        _ = DeepLinkParser.parse(URL(string: "faithform://church/grace-chapel/check-in")!)
    ...
    func malformedLinkFailsClosed() {
        ...
        for bad in [
            "faithform://church/GRACE/check-in",
            "faithform://check-in",
            "faithform://church/grace-chapel/check-in/extra",
    ...
    func emptySegmentsCollapse() {
        ...
        // `faithform://church//check-in` reads `check-in` as the slug on both
        // platforms. It reaches no camera and no check-in: the worst case is a
        // "church not found" screen.
        #expect(
            DeepLinkParser.parse(URL(string: "faithform://church//check-in")!)
