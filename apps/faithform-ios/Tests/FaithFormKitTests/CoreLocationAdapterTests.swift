...
final class FakeCoreLocation: CoreLocationFacade, @unchecked Sendable {
    ...
    // The façade now exposes FaithForm's own enum: the `CLAuthorizationStatus`
...
struct CoreLocationAdapterTests {
    ...
    func registration() async {
        ...
        await adapter.startMonitoring(
            MonitoredRegion(identifier: "faithform.campus.a", latitude: 38.25, longitude: -85.75, radiusMeters: 150)
        )

        let registered = fake.monitoredRegions.compactMap { $0 as? CLCircularRegion }
        #expect(registered.count == 1)
        #expect(registered[0].identifier == "faithform.campus.a")
    ...
    func capacityThroughReconciler() async {
        ...
        let many = (0..<30).map {
            region(String(format: "faithform.campus.%02d", $0), lat: 38 + Double($0) / 100)
        }
    ...
    func regionCallbackTranslation() async throws {
        ...
        #expect(await received.events == [("faithform.campus.a", true)].map { Event($0.0, $0.1) })
    }
