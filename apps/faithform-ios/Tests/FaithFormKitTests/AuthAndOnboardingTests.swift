...
struct InvitationLinkTests {
    ...
    func parsesToken() {
        ...
        let url = URL(string: "faithform://invite/\(token)")!
    ...
    @Test("everything else is refused", arguments: [
        ...
        "faithform://invite/short",                        // too short
        "faithform://invite/aaaaaaaaaaaaaaaa/extra",       // extra segment
        "faithform://invite/aaaaaaaa!aaaaaaaaaa",          // off-alphabet
        "faithform://invite/",                             // empty
    ])
...
struct OnboardingModelTests {
    ...
    func normalizesPastedLink() {
        ...
        #expect(model.normalize("faithform://invite/\(token)") == token)
        #expect(model.normalize("https://faithform.io/faithform/invite/\(token)") == token)
    ...
    func consentOnlyWhenNull() async throws {
        let accepted = try JSONDecoder.faithform.decode(
            Bootstrap.self,
            from: bootstrapJSON(termsVersion: "2026-08-01", privacyVersion: "2026-08-01")
        )
        let fresh = try JSONDecoder.faithform.decode(
            Bootstrap.self,
