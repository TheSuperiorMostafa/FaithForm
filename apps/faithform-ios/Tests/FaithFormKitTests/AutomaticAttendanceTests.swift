...
func pendingUntil(_ date: Date, detectionId: String? = "detection-1") -> AttendanceResult {
    AttendanceResult(
        ...
        occurrenceId: "occ-1",
        countedAt: nil,
        confirmationNotBefore: FaithFormInstant.format(date),
...
func configuration(
    regions: [GeofenceRegion] = [region("faithform.campus.a")],
    version: Int = 7003,
...
struct ReconcilerTests {
    ...
    func respectsRegionLimit() async {
        let many = (0..<35).map { region(String(format: "faithform.campus.%02d", $0), lat: 38 + Double($0) / 100) }
        let location = FakeLocation(authorization: .authorizedAlways)
    ...
    @Test("region ids are FaithForm-scoped and collision-resistant")
    func regionIdsAreScoped() {
        let selected = GeofenceReconciler.selectRegions(
            from: configuration(regions: [region("faithform.campus.6b1f2c48-9d3a-4e51-8b27-0c5a9e4d1f30")])
        )
        #expect(selected.first?.identifier.hasPrefix("faithform.campus.") == true)
...
struct EvidenceTests {
    ...
    func regionEventSubmits() async {
        ...
        let phase = await coordinator.handleRegionEntered(regionId: "faithform.campus.a")
