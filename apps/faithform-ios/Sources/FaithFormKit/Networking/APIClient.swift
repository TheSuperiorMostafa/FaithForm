...
public actor APIClient {
    ...
    public func send<T: Decodable & Sendable>(
        ...
        if let body {
            request.httpBody = try JSONEncoder.faithform.encode(AnyEncodable(body))
    ...
    static func decodeSuccess<T: Decodable & Sendable>(_ type: T.Type, from data: Data) throws -> T {
        do {
            return try JSONDecoder.faithform.decode(MobileSuccess<T>.self, from: data).data
        } catch {
    ...
    static func decodeFailure(from data: Data, requestId: String?, status: Int) -> APIError {
        if let failure = try? JSONDecoder.faithform.decode(MobileFailure.self, from: data) {
            return APIError(
...
extension JSONDecoder {
    ...
    public static let faithform: JSONDecoder = {
...
extension JSONEncoder {
    public static let faithform: JSONEncoder = {
