...
public struct SupabaseAuthClient: SessionAuthenticating {
    ...
    private func post(
        ...
        request.httpBody = try JSONEncoder.faithform.encode(body)
    ...
    private func decodeSession(from data: Data) throws -> StoredSession {
        let decoded = try JSONDecoder.faithform.decode(TokenResponse.self, from: data)
    ...
    static func failure(from data: Data, status: Int) -> AuthFailure {
        let decoded = try? JSONDecoder.faithform.decode(GoTrueError.self, from: data)
