...
public actor KeychainResumePositionStore: ResumePositionStoring {
    ...
    private let prefix = "faithform.resume."
    ...
    private func read(_ partition: CachePartition) -> [ResumePosition] {
        ...
        guard
            ...
            let decoded = try? JSONDecoder.faithform.decode([ResumePosition].self, from: data)
        else { return [] }
        return decoded
    }

    private func write(_ entries: [ResumePosition], partition: CachePartition) {
        guard let data = try? JSONEncoder.faithform.encode(entries) else { return }
    ...
    private func knownPartitions() -> [String] {
        guard
            ...
            let decoded = try? JSONDecoder.faithform.decode([String].self, from: data)
        else { return [] }
        return decoded
    }
    ...
    private func remember(_ storageKey: String) {
        ...
        if let data = try? JSONEncoder.faithform.encode(known) {
