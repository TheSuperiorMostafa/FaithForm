...
import io.faithform.app.giving.GivingFailure
import io.faithform.app.giving.GivingListPhase
import io.faithform.app.giving.GivingScreenState
                    ...
                    if (fund.suggestedAmounts.isNotEmpty()) {
                        ...
                        FlowRow(
                            horizontalArrangement =
                                Arrangement.spacedBy(FaithFormTokens.Spacing.sm),
                        ) {
        ...
        items(state.history, key = { it.attemptId }) { item ->
            Card(modifier = Modifier.fillMaxWidth().semantics(mergeDescendants = true) {}) {
                Row(
                    ...
                    Column(
                        verticalArrangement =
                            Arrangement.spacedBy(FaithFormTokens.Spacing.xs),
                    ) {
                        Text(
...
private fun Centered(modifier: Modifier = Modifier, content: @Composable () -> Unit) {
    Column(
        modifier = modifier.fillMaxWidth().padding(FaithFormTokens.Spacing.xl),
        horizontalAlignment = Alignment.CenterHorizontally,
    ) { content() }
}
