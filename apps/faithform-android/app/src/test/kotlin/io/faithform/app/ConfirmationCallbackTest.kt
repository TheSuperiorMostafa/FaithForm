...
private class CallbackSessions(var session: StoredSession? = null) :
    SessionGateway, io.faithform.app.network.TokenProvider {
...
private const val CALLBACK_URL = "faithform://auth/callback?code=9c1e02f3-4d69-4b8c-a44f-3a9bb75d0a01"
...
class ConfirmationCallbackTest {
        ...
        auth = if (withAuth) {
            SupabaseAuthClient(
                SupabaseAuthConfig(
                    ...
                    anonKey = "anon-key",
                    signUpRedirect = "faithform://auth/callback"
                ),
                transport,
    ...
    fun `a provider failure in the link is its own sentence, with no network`() = runTest {
        ...
        model.handleDeepLink(
            "faithform://auth/callback#error=access_denied&error_code=otp_expired"
        )
