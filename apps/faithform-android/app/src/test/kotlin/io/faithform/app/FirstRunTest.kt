...
import io.faithform.app.ui.auth.AuthUiPhase
...
class FirstRunTest {
    ...
    fun `an invitation deep link signed out is held, never redeemed`() = runTest {
        ...
        val model = viewModel()
        model.handleDeepLink("faithform://invite/$TOKEN_32")
    ...
    fun `a church deep link signed out names the church without joining it`() = runTest {
        ...
        val model = viewModel()
        model.handleDeepLink("faithform://church/grace")
    ...
    fun `an expired invitation surfaces its own failure, not a crash`() = runTest {
        ...
        assertEquals(
            io.faithform.app.contract.MobileErrorCode.INVITATION_EXPIRED,
