 ...
 * wearing its name. It tests that FaithForm *calls* it correctly and handles
 ...
 * a value FaithForm builds, and every field in it is a decision.
 */
...
class GeofencingRequestTest {
    ...
    fun `the request carries the exact geometry it was given`() {
        ...
        assertEquals("faithform.campus.a", fence.requestId)
    ...
    fun `every region in one call, not one call per region`() = runBlocking {
        ...
        val regions = (0 until 5).map { region("faithform.campus.$it") }
    ...
    fun `a partial failure self-heals on the next reconciliation`() = runBlocking {
        ...
        monitor.startMonitoring(listOf(region("faithform.campus.a")))
        assertTrue(monitor.monitoredRegions().isEmpty())
    ...
    fun `removal clears the mirror even when the task fails`() = runBlocking {
        ...
        monitor.stopMonitoring(listOf("faithform.campus.a"))
        ...
        assertTrue(monitor.monitoredRegions().isEmpty())
        assertEquals(listOf(listOf("faithform.campus.a")), facade.removedIds)
    }
