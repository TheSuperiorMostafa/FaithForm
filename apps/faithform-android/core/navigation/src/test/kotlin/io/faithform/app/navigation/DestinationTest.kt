...
class CheckInRouteTest {
    ...
    fun `the deep link parses to a check-in destination`() {
        assertEquals(
            ...
            DeepLinkParser.parse("faithform://church/grace-chapel/check-in"),
        )
    }
    ...
    fun `empty path segments collapse, on both platforms alike`() {
        // Documented rather than asserted-away. `faithform://church//check-in`
        ...
        assertEquals(Destination.Church("check-in"), DeepLinkParser.parse("faithform://church//check-in"))
    }
}
