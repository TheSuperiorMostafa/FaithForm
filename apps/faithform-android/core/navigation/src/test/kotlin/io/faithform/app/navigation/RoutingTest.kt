...
class RoutingTest {
    ...
    fun `valid links parse to typed destinations`() {
        assertEquals(Destination.Home, DeepLinkParser.parse("faithform://home"))
        assertEquals(Destination.Account, DeepLinkParser.parse("faithform://account"))
        assertEquals(Destination.AccountPrivacy, DeepLinkParser.parse("faithform://account/privacy"))
        assertEquals(Destination.ChurchDiscovery, DeepLinkParser.parse("faithform://discover"))
        assertEquals(Destination.Church("grace-community"), DeepLinkParser.parse("faithform://church/grace-community"))
        assertEquals(Destination.Watch("grace"), DeepLinkParser.parse("faithform://church/grace/watch"))
        assertEquals(Destination.Give("grace"), DeepLinkParser.parse("faithform://church/grace/give"))
    }
    ...
    fun `unknown malformed and hostile links fail closed`() {
        val bad = listOf(
            ...
            "faithform://nope",
            "faithform://church",
            "faithform://church/GRACE",
            "faithform://church/../../etc/passwd",
            "faithform://church/grace/unknown",
            "faithform://church/grace/watch/extra",
    ...
    fun `a link is authorized before anything is mutated`() {
        assertEquals(
            RouteResolution.Rejected(RouteRejection.NOT_IMPLEMENTED),
            registry.resolve("faithform://church/grace/give", snapshot())
        )
    }
}
