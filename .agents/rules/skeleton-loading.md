...
8. **Static-First Principle (Text That Doesn't Change Loads Normally)**: Text and chrome that never change (page titles, descriptions, static buttons, section labels, table column headers, weekday names) MUST load normally as real text without skeleton shimmer. Skeletons should ONLY mask dynamic, data-dependent content.
