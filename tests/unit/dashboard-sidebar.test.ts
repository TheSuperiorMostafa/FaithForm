...
test("the sidebar collapses to the rail at every screen size", () => {
  assert.doesNotMatch(sidebar, /pinned|lg:w-64|lg:max-w-full/);
  assert.doesNotMatch(shell, /lg:ml-/);
  // Sign out is still labelled when the panel is open.
