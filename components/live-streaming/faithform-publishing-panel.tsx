...
import {
  getFaithFormPublicationHistory,
  getFaithFormPublishingState,
  getPosterChoicesFor,
  previewFaithFormVisibility,
  publishMediaToFaithForm,
  unpublishMediaFromFaithForm,
} from "@/app/dashboard/live-streaming/faithform-actions";
...
/**
 * Publishing a service to the FaithForm apps.
 *
...
const STATE_LABELS: Record<string, { label: string; tone: "live" | "on" | "off" | "wait" }> = {
  draft: { label: "Not in FaithForm", tone: "off" },
  ...
  published: { label: "In FaithForm", tone: "on" },
  unpublished: { label: "Removed from FaithForm", tone: "off" },
...
export function FaithFormPublishingPanel() {
  ...
  const [preview, setPreview] = useState<Awaited<
    ReturnType<typeof previewFaithFormVisibility>
  > | null>(null);

  const refresh = useCallback(() => {
    startTransition(async () => {
      const result = await getFaithFormPublishingState();
      if (result.ok) {
  ...
  if (!loaded) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-base">FaithForm app</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base">FaithForm app</CardTitle>
        <CardDescription>
          Nothing appears in the FaithForm app until you publish it here. A
      ...
      <CardContent className="flex flex-col gap-4">
        {!configured && (
          ...
          >
            Publishing to FaithForm isn&apos;t set up on this FaithForm
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            onClick={() =>
              startTransition(async () => {
                const result = await previewFaithFormVisibility();
        ...
        {items.length === 0 ? (
          ...
          <ul className="flex flex-col divide-y divide-border">
            {items.map((item) => {
              ...
              // whether FaithForm can honour it. A recording that was published
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      ...
                      onClick={() =>
                        startTransition(async () => {
                          const result = await getFaithFormPublicationHistory({
                            kind: item.kind,
                            id: item.id,
                          });
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Publish to FaithForm</DialogTitle>
          <DialogDescription>
        ...
        <div className="flex flex-col gap-4">
          ...
          <fieldset className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Choose one of this church&apos;s own images. FaithForm shows a
              typographic card when there is none.
            </p>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="radio"
                name="faithform-poster"
          ...
          {item.kind === "recording" && (
            <div className="flex flex-col gap-2">
              <Label htmlFor="faithform-summary">What this service was about</Label>
              <Textarea
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() =>
              startTransition(async () => {
                const result = await publishMediaToFaithForm({
                  kind: item.kind,
                  id: item.id,
                  visibility,
                  posterUrl,
                ...
                if (result.ok) {
                  toast.success("Published to FaithForm.");
                  onDone();
                } else {
                  toast.error(result.error);
                }
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {revoke ? "Revoke access to this?" : "Remove this from FaithForm?"}
          </DialogTitle>
          <DialogDescription>
            {revoke ? (
              <>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() =>
              startTransition(async () => {
                const result = await unpublishMediaFromFaithForm({
                  kind: item.kind,
                  id: item.id,
                  revoke,
                });
                if (result.ok) {
                  toast.success(revoke ? "Access revoked." : "Removed from FaithForm.");
                  onDone();
                } else {
                  toast.error(result.error);
                }
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        ...
        {value.entries.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            This has never been published to FaithForm.
          </p>
        ) : (
...
function PreviewDialog({
  ...
  value: Awaited<ReturnType<typeof previewFaithFormVisibility>> | null;
  onClose: () => void;
}) {
