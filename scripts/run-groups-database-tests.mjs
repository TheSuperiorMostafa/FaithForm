 ...
 * Groups and Messaging database suite against it.
 *
...
const database = `ff_groups_${randomUUID().slice(0, 8)}`;
...
try {
  ...
  const result = spawnSync(
    "npx",
    // One file at a time: both suites share this database, and the outbox
    // tests reason about jobs other suites' triggers would also create.
    [
      "tsx",
      "--test",
      "--test-concurrency=1",
